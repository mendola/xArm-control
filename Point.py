...
class Point:
    ...
    def __init__(self, *,
                 cartesian: Optional[Tuple[float, float, float]] = None,
                 cylindrical: Optional[Tuple[float, float, float]] = None,
                 spherical: Optional[Tuple[float, float, float]] = None):
        """
            Initialize a point in space.
        :param cartesian: Tuple(x, y, z)
        :param cylindrical: Tuple(polar_radius, polar_angle, z)
        :param spherical: Tuple(spherical_radius, azimuth, polar_angle)
        """
    ...
    @classmethod
    def cart2cyl(cls, x: float, y: float, z: float) -> Tuple[float, float, float]:
        """ Converts Cartesian to Cylindrical """
        return cls.from_cartesian(x, y, z)[1]

    @classmethod
    def cart2sphere(cls, x: float, y: float, z: float) -> Tuple[float, float, float]:
        """ Converts Cartesian to Spherical """
        return cls.from_cartesian(x, y, z)[2]

    @classmethod
    def cyl2cart(cls, polar_radius: float, polar_angle: float, z: float) -> Tuple[float, float, float]:
        """ Converts Cylindrical to Cartesian """
        return cls.from_cylindrical(polar_radius, polar_angle, z)[0]

    @classmethod
    def cyl2sphere(cls, polar_radius: float, polar_angle: float, z: float) -> Tuple[float, float, float]:
        """ Converts Cylindrical to Spherical """
        return cls.from_cylindrical(polar_radius, polar_angle, z)[2]

    @classmethod
    def sphere2cart(cls, spherical_radius: float, azimuth: float, polar_angle: float) -> Tuple[float, float, float]:
        """ Converts Spherical to Cartesian """
        return cls.from_spherical(spherical_radius, azimuth, polar_angle)[0]

    @classmethod
    def sphere2cyl(cls, spherical_radius: float, azimuth: float, polar_angle: float) -> Tuple[float, float, float]:
